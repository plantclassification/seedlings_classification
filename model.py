...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for x, y in test_loader:
            ...
            if type(hat) is int:
                hat = pred.view(-1).cpu().numpy()
            else:
                hat = np.concatenate((hat, pred.view(-1).cpu().numpy()), 0)
            if type(true) is int:
                true = y.cpu().numpy()
            else:
                true = np.concatenate((true, y.cpu().numpy()), 0)
    print(confusion_matrix(true, hat))
...
def predict_1(model, device, input):
    model.eval()
    with torch.no_grad():
        x = input.to(device)
        x = x.float()
...
if __name__ == '__main__':
    ...
    def prediction():
        ...
        model.load_state_dict(torch.load('./model.pth'))
        predictions = predict(model, 'cuda', predict_set)
        predict_data['label'] = predictions

        with open('submission.csv','w',encoding='utf-8') as f:
            f.write('file,species'+'\n')
            for i in range(len(predict_data['id'])):
                f.write(predict_data['id'][i] + ',' + INV_CLASS[predictions[i]] + '\n')
